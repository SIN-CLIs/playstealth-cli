...
def _looks_like_selector(value: str) -> bool:
    """Return True when a target likely is already a CSS selector."""
    return value.startswith(("#", ".", "[", "input", "button", "select", "textarea", "a", "text="))
...
class SmartClickAction:
    ...
    async def resolve(
        self, page_or_target: Page | str, target_text: str | None = None, timeout: int = 5000
    ):
        """Resolve a target into a visible locator.

        Backward compatibility:
        - `SmartClickAction(page).resolve("Weiter")`
        - `SmartClickAction().resolve(page, "Weiter")`
        """
        if isinstance(page_or_target, Page):
            page = page_or_target
            if target_text is None:
                raise ValueError("target_text is required when page is passed explicitly")
        else:
            if self.page is None:
                raise ValueError("page must be provided")
            page = self.page
            target_text = str(page_or_target)

        selectors = [
            target_text if _looks_like_selector(target_text) else None,
            f'[aria-label="{target_text}"]',
            f'[data-testid="{target_text}"]',
    ...
    async def execute(
        ...
        resolved = await self.resolve(page, target_text, timeout)
        if resolved is not None:
            selector = await resolved.evaluate(
                "el => el.id ? `#${el.id}` : (el.getAttribute('aria-label') ? `[aria-label=\"${el.getAttribute('aria-label')}\"]` : '')"
            )
            if selector:
                return await human_click(page, selector)
            await resolved.click(timeout=timeout)
            return True

        # Strategy 1: Aria-label exact match
        selector = f'[aria-label="{target_text}"]'
        if await self._try_click(page, selector, timeout):
            return True
            ...
            try:
                ...
                if text and target_text.lower() in text.lower():
                    if await btn.is_visible():
                        return await human_click(page, f"button:has-text('{text[:50]}')")
            except Exception:
                continue
            ...
            try:
                ...
                if text and target_text.lower() in text.lower():
                    if await link.is_visible():
                        return await human_click(page, f"a:has-text('{text[:50]}')")
            except Exception:
                continue
        ...
        for inp in inputs:
            try:
                ...
                if (
                    target_text.lower() in value.lower()
                    or target_text.lower() in placeholder.lower()
                ):
                    if await inp.is_visible():
                        await inp.scroll_into_view_if_needed(timeout=2000)
                        await idle_time(page, 0.5, 0.2)
        ...
        clickable_elements = await page.query_selector_all("[onclick]")
    ...
    async def _try_click(self, page: Page, selector: str, timeout: int) -> bool:
...
class SmartTypeAction:
    ...
    async def execute(
        ...
        if isinstance(page_or_target, Page):
            page = page_or_target
            if target_identifier is None or text_to_type is None:
                raise ValueError(
                    "target_identifier and text_to_type are required when page is explicit"
                )
        else:
            if self.page is None or target_identifier is None:
                raise ValueError("page and text are required")
            page = self.page
            text_to_type = target_identifier
            target_identifier = str(page_or_target)

        # Strategy 1: Label text match
        if _looks_like_selector(target_identifier):
            direct = await page.query_selector(target_identifier)
            if direct and await direct.is_visible():
                return await human_type(page, target_identifier, text_to_type)
                ...
                if label_text and target_identifier.lower() in label_text.lower():
                    ...
                    if for_attr:
                        ...
                        if input_elem and await input_elem.is_visible():
                            return await human_type(page, f"#{for_attr}", text_to_type)
                ...
                if placeholder and target_identifier.lower() in placeholder.lower():
                    if await inp.is_visible():
                        return await human_type(
                            page, self._get_selector_for_element(inp), text_to_type
                        )
            except Exception:
                continue
        ...
        for elem in elements:
            try:
                if await elem.is_visible():
                    return await human_type(
                        page, self._get_selector_for_element(elem), text_to_type
                    )
            except Exception:
                continue
...
class SmartSelectAction:
    ...
    async def execute(
        ...
        if isinstance(page_or_target, Page):
            ...
            if target_identifier is None:
                raise ValueError("target_identifier is required when page is explicit")
        else:
            if self.page is None:
                raise ValueError("page must be provided")
            page = self.page
        ...
        select_elem = await self._find_select_element(page, target_identifier)
    ...
    async def _find_select_element(self, page: Page, target_identifier: str):
        ...
        for label in labels:
            try:
                ...
                if label_text and target_identifier.lower() in label_text.lower():
                    for_attr = await label.get_attribute("for")
                    if for_attr:
                        select_elem = await page.query_selector(f"select#{for_attr}")
